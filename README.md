...
O AMCL localiza o robô no mapa salvo e o `full_coverage_path_planner` + MBF +
`tracking_pid` percorrem a área. Para encerrar tudo: `Ctrl+C` no Terminal 2 e
depois no Terminal 1.
...
| `cloud` (fase 2) | `pointcloud_to_laserscan` | Projeta a nuvem 3D (`/camera/depth/points`) com filtro de altura. Requer `ros-noetic-pointcloud-to-laserscan`. |
...
salva `maps/mapa_atual.{yaml,pgm}` com `rosrun map_server map_saver -f
.../maps/mapa_atual` seguido de `rosrun robo_aspirador_kinect fechar_mapa.py
.../maps/mapa_atual.yaml` (ou pela opção `s` do menu, na alternativa
